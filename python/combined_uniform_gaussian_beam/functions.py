...
def create_G_mnij(
    ...
    kR = k * R  # (N, N)
    G = cp.exp(1j * kR) / (4 * cp.pi * (R + a) * epsilon_0 * epsilon_b)  # (N, N)
    ...
    scaling_terms = 1 / ((R[:, :, None, None] + a) ** 4)
...
def create_G_field(
    ...
    G_scalar = cp.exp(1j * kR) / (4 * cp.pi * (R + a) * epsilon_0 * epsilon_b)
    ...
    G_field = (main - off) / ((R[:, :, None, None] + a) ** 4)
...
def gen_Einc_mi_gaussian(pos_arr: NDArray[float64]) -> NDArray[complex128]:
    ...
    Einc_mi[:, 0] = (
        cp.cos(pol_angle) * E0 * cp.exp(1j * k * z) * cp.exp(-(x**2 + y**2) / w0**2)
    )
    Einc_mi[:, 1] = (
        cp.sin(pol_angle) * E0 * cp.exp(1j * k * z) * cp.exp(-(x**2 + y**2) / w0**2)
    )
